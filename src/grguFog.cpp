...
grFogTable( const GrFog_t *ft )
{
    ...
    {
        MMX_memcpy( Glide.FogTable, (GrFog_t *)ft, GR_FOG_TABLE_SIZE * sizeof( uint8_t ) );
        Glide.FogTable[ GR_FOG_TABLE_SIZE ] = 255;
		
        for ( int i = 0; i < GR_FOG_TABLE_SIZE; i++ )
        {
            for ( uint32_t j = intStartEnd[ i ]; j < intStartEnd[ i + 1 ]; j++ )
            {
                OpenGL.FogTable[ j ] = (uint8_t)( Glide.FogTable[ i ] + 
                     ...
                     float nearZ, float farZ )
{
    ...
    for( i = Start; i <= End; i++ )
    {
        fogtable[ i ] = (uint8_t)((float)( End - Start ) / 255.0f * (float)( i - Start ));
    }
